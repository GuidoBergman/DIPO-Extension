...
function markElementsWithContentModelRestrictions(){
 var elements = document.querySelectorAll("h1, h2, h3, h4, h5, h6, figcaption, .pirulo-block-middle");
...
function highlightRestrictedElements(text, techniqueName) {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    elementText = elementText.replace(new RegExp(`[“”]`),`"`);
...
function fixClonedDocument(document){
  ...
  var elements = document.querySelectorAll("h1, h2, h3, h4, h5, h6, p, time, figcaption, bsp-timestamp");
  for (var i=0, max=elements.length; i < max; i++) {
...
async function classifyText() {
  ...
  var text;
  // If the expert is the same as the title or is included in text, ignore it
  if ((article.title != article.excerpt) &&
      (! article.textContent.includes(article.excerpt.substring(0, article.excerpt.length - 3)))
      ) {
